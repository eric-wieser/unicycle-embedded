                         ...
                         float &dphi, float &dtheta, float &dpsi)
{
  ...
  q = p*q; q.normalize();               // new unit quaternion = previous position q + local integration p
  
  quat p1( 1, w[0]/2*dt/10, w[1]/2*dt/10, w[2]/2*dt/10); // quaternion for the instantaneous position change
  quat q1 = p1*q; q1.normalize();       // unit quaternion to represent the next position after a tiny timestep forward
  ...
  dphi = (phi1-phi)/(dt/10); dtheta = (theta1-theta)/(dt/10); dpsi = (psi1-psi)/(dt/10); // instantaneous Euler velocities
